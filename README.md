...
```bash
conda create -n kb_devops_proj python=3.9
conda activate kb_devops_proj
```
...
```
This command will create your personal repository under `/Repos/<username>/kb_devops_proj`.
...
- To trigger the release pipeline, get the current version from the `kb_devops_proj/__init__.py` file and tag the current code version:
```
